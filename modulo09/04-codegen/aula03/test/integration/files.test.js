...
function generatedFilePath({ mainPath, defaultMainFolder, layers, componentName }) {
    return layers.map(layer => {
        // factory
        // /Documents/project/src/factory/herosFactory.js
        const filename = `${componentName}${Util.upperCaseFirstLetter(layer)}.js`
        // mainPath: /Document/project/jsexpert
        // defaultMainFolder: src
        // layer: factory
        // filename: herosFactory.js
...
describe('#Integration - Files - Files Structure', () => {
    ...
    // como não obtivemos o caminho relativo estamos pensando que o comando vai rodar do package.json que está na raiz
    // por isso, iniciamos pegando da pasta test
    const packageJSON = 'package.json'
    const packageJSONLocation = join('./test/integration/mocks', packageJSON)

    function getAllFunctionsFromInstances(instance) {
    ...
    test('Repository class should have create, read, update and delete methods', async () => {
        ...
        const [ repositoryFile ] = generatedFilePath(myConfig)
    ...
    test('Service should have the same signature of repository and call all its methods', async () => {
        ...
        await createFiles(myConfig)
        const [ repositoryFile, serviceFile ] = generatedFilePath(myConfig)
        ...
        const allRepositoryMethods = getAllFunctionsFromInstances(repository)
    ...
    test('Factore instance should match layers', async () => {
        ...
        const [ factoryFile, repositoryFile, serviceFile ] = generatedFilePath(myConfig)
